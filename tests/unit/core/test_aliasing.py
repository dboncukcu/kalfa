...
@pytest.mark.parametrize("name", ["01_mlp_regression", "alad", "07_wgan_gp", "10_char_lm"])
def test_the_std_flow_has_no_aliasing_hazard(name, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    prepared = prepare([example(name)], parse_sets([]))
