...
def test_every_example_describes(name, capsys):
    code = main(["describe", example(name)])
    out = capsys.readouterr().out
    assert code in (0, 1)
    for head in HEADS:
        assert head in out, f"{name} has no {head.strip()} section"
...
def test_describe_shows_the_target_table_and_the_slots(tmp_path, monkeypatch, capsys):
    ...
    assert main(["describe", example("15_multi_target")]) == 0
    out = capsys.readouterr().out
...
def test_describe_load_falls_back_to_the_plan_without_a_header(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.chdir(tmp_path)
    config = Path(example("15_multi_target")).read_text().replace(
...
def test_describe_does_not_clip_when_the_output_is_not_a_terminal(workdir, capsys):
    assert main(["describe", example("15_multi_target")]) == 1        # no scores.parquet here
    out = capsys.readouterr().out
