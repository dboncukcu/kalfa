...
def test_set_target_and_set_value(workdir):
    ...
    config["training"]["rules"] = [
        ...
        {"name": "c", "when": {"uri": "after_epoch", "params": {"at": 1}}, "set": {"loss_mse.delta": 1.0}},
        {"name": "d", "when": {"uri": "after_epoch", "params": {"at": 1}}, "set": {"net.trainable": "yes"}},
        {"name": "e", "when": {"uri": "after_epoch", "params": {"at": 1}}, "set": {"ghost.loss": "loss_mse"}},
        {"name": "f", "when": {"uri": "after_epoch", "params": {"at": 1}}, "set": {"net.lr": 0.1}},
    ]
    prepared, kinds = kinds_of(workdir, config)
...
def test_unused_optimizer(workdir):
    config = minimal()
    config["optimizers"] = {"spare": {"uri": "adam", "loss": "loss_mse"}}
    prepared, kinds = kinds_of(workdir, config)
    assert "unused_optimizer" in kinds


def two_models(config, optimizer="main"):
    net = config["model"]["models"].pop("net")
    del net["optimizer"]
    config["model"] = {"models": {"a": {**net, "optimizer": optimizer}, "b": {**net, "optimizer": optimizer}}}
    del config["training"]["loss"]
    return config


def test_predicts_required(workdir):
    config = two_models(minimal())
    config["optimizers"] = {"main": {"uri": "adam", "loss": "loss_mse"}}
    prepared, kinds = kinds_of(workdir, config)
...
def test_model_name_dot_and_reserved(workdir):
    config = minimal()
    net = config["model"]["models"].pop("net")
    config["model"]["models"] = {"a.b": net, "training": dict(net)}
...
def test_turn_order(workdir):
    ...
    config = two_models(minimal())
    config["model"]["models"]["a"]["optimizer"] = "g"
    config["model"]["models"]["b"]["optimizer"] = "d"
    config["optimizers"] = {"g": {"uri": "adam", "loss": "loss_mse"}, "d": {"uri": "adam", "loss": "loss_mse"}}
...
def test_warnings_no_seed_unused_preprocessor_untrained_model(workdir):
    ...
    net = config["model"]["models"].pop("net")
    del net["optimizer"]
    config["model"] = {"models": {"model": {**net, "optimizer": "main"}, "idle": dict(net)}}
    config["optimizers"] = {"main": {"uri": "adam", "loss": "loss_mse"}}
...
def test_empty_plots_and_metrics_are_empty_groups(workdir):
    ...
    config["plots"] = {}
    config["metrics"] = {}
    config["training"]["stop"] = []
    config["training"]["checkpoint"] = {"uri": "best", "params": {"monitor": "val/loss_mse"}}
    prepared, kinds = kinds_of(workdir, config)
...
def test_reserved_field_and_duplicate_name(workdir):
    ...
    config["metrics"]["loss_mse"] = {"uri": "mse"}
    prepared, kinds = kinds_of(workdir, config)
...
def test_definition_keys_are_accepted_and_plot_inputs_are_typed(workdir):
    config = minimal()
    config["data"]["preprocessors"]["std_scaler"] = {"uri": "standard_scaler", "sets": ["train"]}
    ...
    assert document["flow"]["data"]["params"]["preprocessors_keys"] == {"std_scaler": {"sets": ["train"]},
                                                                        "target_std": {}}
...
def two_targets():
    config = minimal()
    config["data"]["fields"] = {"x0": {"target": True, "preprocessors": ["std_scaler"]},
                                "x*": {"preprocessors": ["std_scaler"]},
                                "price": {"target": True, "preprocessors": ["target_std"]}}
    net = config["model"]["models"]["net"]
    net["outputs"] = ["y", "y2"]
    net["nodes"] = {"y": {"uri": "linear", "params": {"out_features": 1}, "inputs": ["x"]},
...
def test_a_definition_without_a_target_is_caught(workdir):
    ...
    prepared, kinds = kinds_of(workdir, config)
    assert kinds == ["target_missing", "target_missing"]
