...
def test_check_folds_and_the_valid_less_form(dataset):
    dataset("11_kfold_cv")
    ...
    prepared = check(["config.yaml"], parse_sets([no_valid]))
    ...
    prepared = check(["config.yaml"], parse_sets([no_valid, "training.stop=[]", "training.checkpoint=last",
    ...
    prepared = check(["config.yaml"], parse_sets([no_valid, "training.stop=[]", "training.rules=[]",
...
def test_five_folds_and_collect(dataset):
    dataset("11_kfold_cv")
    tests = set()
    for fold in range(5):
        ...
        rows = set(pandas.read_parquet(Path(result.record) / "predictions.parquet")["row"])
