...
record to end after its current turn), explores the test predictions, the architecture as a schematic (a block
clicked opens in place into its layers, with the tensor shapes on the wires between them), the data pipeline (a stage clicked
...
| `history.jsonl` | per turn the `train/`, `val/`, `test/` values (`val/` steers the checkpoint, the stop and the rules, `test/` is only reported), `global_step`, `lr/<optimizer>`, `minimizes/<optimizer>` (the loss it stepped on that turn), `effect/<target>` (every other effect of the rules in force that turn), `seconds`, the rules that fired |
| `architecture.json` | the graph of every report model: the boxes of the architecture drawing with their column and row, the traced shapes and the wires, the layers inside every node as a tree with their shapes and parameter counts; the board draws it as a schematic and opens a block into its layers |
