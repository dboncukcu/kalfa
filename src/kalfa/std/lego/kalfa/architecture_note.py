...
def layout_note(layout):
    ...
    return {"boxes": boxes, "arrows": [list(arrow) for arrow in layout.arrows]}
...
def architecture_note(models, loaders, composites=None, predicts=None, losses=None, losses_keys=None, optimizers=None,
                      record=None, device=None):
    ...
    if record is not None:
        write_json(Path(record) / "architecture.json", {"models": note})
    return None
