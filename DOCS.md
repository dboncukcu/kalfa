...
| `/lego/kalfa/architecture_note` |  | `(models, loaders, composites=None, prep=None, predicts=None, losses=None, losses_keys=None, optimizers=None, record=None, device=None)` | returns: None; bus: record=record, device=device | The graph of every report model written to the record as architecture.json: the boxes of the architecture drawing with their column and row, the shapes one batch traced and the wires as arrows; the board draws it |
