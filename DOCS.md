...
| `/plot/kalfa/pred_vs_true` | `pred_vs_true` | `(predictions, history, models, record, name=None, columns=4)` | partial: True | Predicted against true values of the test set, one panel per predicted field, laid out in a grid of columns panels per row and titled with the field name, plus the output wire when two outputs predict the same field |
