...
| `/plot/kalfa/pred_vs_true` | `pred_vs_true` | `(predictions, history, models, record, name=None)` | partial: True | Predicted against true values of the test set, one panel per target |
...
| `/lego/kalfa/predict` |  | `(models, composites, loader, prep, predicts, set, record=None, device=None)` | returns: predictions; bus: record=record, device=device | Predict the test set with the report model, invert the target chain, write predictions.parquet |
